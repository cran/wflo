...
The central function to \textbf{wflo} is \verb"Profit()". It takes a set of points in the unit square (turbine locations) and based on the adjusted AEP in the farm specified via the \verb"e$FarmVars" settings object, checks whether the layout is valid (i.e. minimum distances are met for all turbines), computes multiple Jensen wake penalties, generates the total marketable power production of the specified layout, takes cost and sale price into consideration and finally computes the farm's economic profit. Since most numeric optimizers by default operate as a minimizer, \verb"Profit()" returns the negative profit, i.e. a negative number for positive actual profit values and a positive number if cost is greater than revenue (negative actual profit).\\
